...
function ProductCard({
  ...
  return (
    ...
    >
      {/* Close Button (shows only when expanded) */}
      {expanded && (
        <button
          className="absolute top-3 right-3 z-30 bg-gray-200 hover:bg-gray-300 rounded-full p-1 text-gray-700"
          onClick={(e) => {
            e.stopPropagation();
            if (onCollapse) {
              onCollapse();
      ...
      {/* Left: Image */}
      <div
        className={`w-full md:w-[45%] relative rounded-md mb-3 md:mb-0 overflow-hidden ${
      ...
      <div className="w-full md:w-[55%] flex flex-col justify-start items-start md:pl-6 h-auto md:h-full md:pt-[10px]">
        <div className="text-lg md:text-2xl font-bold italic text-[#3b4381] mb-2 md:mb-[20px]">
          {title}
        </div>
        {product && (
          <div className="text-lg font-medium md:text-xl text-[#3b4381] mb-2 md:mb-[20px]">
            {product}
          </div>
        )}
        ...
        <AnimatePresence>
          {expanded && (
            ...
            >
              {description1 && (
                <div className="mt-2 text-[#3b4381] text-sm md:text-base">
                  {description1}
                </div>
              )}
              {keybenefits && keybenefits.length > 0 && (
                <ul className="mt-2 list-disc pl-5 text-[#3b4381] text-sm md:text-base">
                  <p className="font-semibold my-4">Key Benefits:</p>
